...
map<StateSet, map<char, StateSet>> convertNFAtoDFA(const map<string, pair<string, string>> &nfaTransitions)
{
    ...
    map<StateSet, string> stateNameMap;                // Mapping of state sets to state names
    vector<StateSet> unmarkedStates;                   // States to process
    int stateCounter = 0;                              // Counter for naming DFA states
    ...
    stateNameMap[startState] = "q" + to_string(stateCounter++);
    ...
    while (!unmarkedStates.empty())
    {
        ...
        for (char input : {'0', '1'})
        {
            ...
            cout << endl;
            // If newSet is non-empty, add it as a new DFA state if it hasn't been seen before
            if (!newSet.empty())
            {
                if (stateNameMap.find(newSet) == stateNameMap.end())
                {
                    stateNameMap[newSet] = "q" + to_string(stateCounter++);
